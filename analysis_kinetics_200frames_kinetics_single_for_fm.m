function [C,D]=analysis_kinetics_200frames_kinetics_single_for_fm(Q,x,y)% Q is one Column of the original data from Metamorph, x is the number of frames we want to analyze after 20s
...
C=zeros(x,n); g=1;h=1;l=y+1;% C is the matrix of data after normalized with baseline after 20s, the first 20s' normalized intensity is not needed for the exponential decay fitting
while g<=x
    C(g,h)=g;
    g=g+1;
end
...
while r<=c
    [f,G]=fit(C(1:s,1),C(1:s,r),'a+b*exp(-d*x)');
