function [ C ] = analysis_intensity_200frames_trace_for_Cal520( Q )
% work as to generate every single normalized fluorescence trace as a [200,x] matrix C
% input Q is read out of metamorph software from Cal520 calcium imaging raw data
...
while h<=n-1
    while g<=m
        C(g,h)=B(g,h)/k(1,h)-1;%-1 works for ¦¤F to make baseline start from 0
        g=g+1;
    end
    g=1;h=h+1;
end
